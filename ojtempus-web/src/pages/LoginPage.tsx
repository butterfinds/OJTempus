...
export function LoginPage() {
  ...
  const handleEmailAuth = async (e: FormEvent) => {
    ...
    try {
      ...
      const hasConfig = await hasOJTConfig(user?.id)
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Note: User will be redirected by Supabase OAuth flow, then handled by auth callback
    } catch (err: unknown) {
    ...
    <AuthLayout>
      ...
      <div className="mt-8 text-center">
        <p className="text-gray-500 text-sm font-medium">
          ...
          <Link to="/signup" className="text-[#44ACFF] font-bold hover:text-[#3A9BEF] active:text-[#2E8BDF] transition-colors hover:underline">
            Sign up
          </Link>
        </p>
      </div>
